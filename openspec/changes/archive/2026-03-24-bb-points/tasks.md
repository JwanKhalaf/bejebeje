...
- [x] 1.1 **[BLOCKED — user must create migration]** A database migration is required. Create `users` table with columns: id (int, PK, auto-increment), cognito_user_id (text, UNIQUE, NOT NULL), username (text, UNIQUE, NOT NULL), artist_submission_points (int, NOT NULL, DEFAULT 0), artist_approval_points (int, NOT NULL, DEFAULT 0), lyric_submission_points (int, NOT NULL, DEFAULT 0), lyric_approval_points (int, NOT NULL, DEFAULT 0), report_submission_points (int, NOT NULL, DEFAULT 0), report_acknowledgement_points (int, NOT NULL, DEFAULT 0), last_seen_points (int, NOT NULL, DEFAULT 0), created_at (timestamp, NOT NULL), modified_at (timestamp, NULL). Add UNIQUE indexes on cognito_user_id and username. Ref: SPEC §5 New Entity: User.
- [x] 1.2 **[BLOCKED — user must create migration]** A database migration is required. Create `point_events` table with columns: id (int, PK, auto-increment), user_id (int, FK to users.id, NOT NULL), action_type (int, NOT NULL), points (int, NOT NULL), entity_id (int, NOT NULL), entity_name (text, NOT NULL), created_at (timestamp, NOT NULL). Add index ix_point_events_user_id_created_at on (user_id, created_at DESC). Add UNIQUE index ix_point_events_user_id_action_type_entity_id on (user_id, action_type, entity_id). Ref: SPEC §5 New Entity: PointEvent.
...
- [x] 16.1 Create a new `Bejebeje.Retroactive` console application project in the solution. It SHALL accept a connection string via command-line argument or environment variable and connect to the PostgreSQL database. Ref: SPEC §10 Retroactive Console Application.
- [x] 16.2 Implement the distinct user_id collection: query all distinct user_id values from artists, lyrics, lyric_reports, and likes tables. Ref: SPEC §7 Flow 12 step 3.
- [x] 16.3 Implement username resolution: for each user_id, call the Cognito AdminGetUser API to resolve preferred_username. On failure, use fallback username "user-{first8charsOfUserId}". Ref: SPEC §7 Flow 12 step 4a.
- [x] 16.4 Implement per-category point computation: artist_submission_points = SUM(CASE WHEN has_image THEN 5 ELSE 1 END) from non-deleted artists; artist_approval_points = SUM(CASE WHEN has_image THEN 10 ELSE 9 END) from approved non-deleted artists; lyric_submission_points = 5 * COUNT from non-deleted lyrics; lyric_approval_points = 15 * COUNT from approved non-deleted lyrics; report_submission_points = 1 * COUNT from non-deleted lyric_reports; report_acknowledgement_points = 4 * COUNT from acknowledged (status=1) non-deleted lyric_reports. Ref: SPEC §7 Flow 12 step 4b.
- [x] 16.5 Implement like-based points computation: floor(COUNT(*) FROM likes WHERE user_id = X / 10). Ref: SPEC §7 Flow 12 step 4c.
- [x] 16.6 Implement UPSERT into users table: INSERT ON CONFLICT (cognito_user_id) UPDATE, set all category columns and last_seen_points = total (prevents false indicator on first login). No rows inserted into point_events. Ref: SPEC §7 Flow 12 steps 4e, 5.
- [x] 16.7 Implement progress logging: log each user processed, final summary (total users processed, total points awarded), and any errors to stdout. Exit code 0 on success, non-zero on failure. Ref: SPEC §7 Flow 12 step 6, §10 Retroactive Console Application.
- [x] 16.8 Implement point_events detection: at startup, check if point_events table has any rows. If so, log a warning. If running post-launch, sum existing point_events per user and add to retroactive totals to avoid overwriting. Ref: SPEC §7 Flow 12 Idempotency.
