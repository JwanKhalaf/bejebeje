...
public class LyricsService : ILyricsService
{
  ...
  public async Task<ArtistLyricsViewModel> GetLyricsAsync(string artistSlug, string userId)
  {
    ...
    {
      string sqlCommand =
        "select l.title as lyric_title, lslugs.name as lyric_slug, l.is_approved, l.is_verified from lyrics as l inner join lyric_slugs as lslugs on lslugs.lyric_id = l.id where case when @user_id <> '' then l.user_id = @user_id or l.is_approved = true else l.is_approved = true end and artist_id = @artist_id and l.is_deleted = false and lslugs.is_primary = true order by l.title asc;";
  ...
  public async Task<IEnumerable<SearchLyricResultViewModel>> SearchLyricsAsync(string title)
  {
    ...
    await using NpgsqlCommand command = new NpgsqlCommand(
      "select l.title as lyric_title, ls.name as lyric_primary_slug, a.first_name artist_first_name, a.last_name artist_last_name, \"as\".name artist_primary_slug from lyrics l inner join lyric_slugs ls on ls.lyric_id = l.id inner join artists a on l.artist_id = a.id inner join artist_slugs \"as\" on a.id = \"as\".artist_id where l.is_deleted = false and l.is_approved = true and \"as\".is_primary = true and ls.name like @lyric_title;",
      connection);
  ...
  public async Task<LyricDetailsViewModel> GetSingleLyricAsync(string artistSlug, string lyricSlug, string userId)
  {
    ...
    viewModel.Author = new AuthorViewModel();

    await using NpgsqlConnection connection = new NpgsqlConnection(_databaseOptions.ConnectionString);
    await connection.OpenAsync();

    await using NpgsqlCommand command = new NpgsqlCommand(
      "select l.id, l.title, l.body, count(likes.lyric_id) as number_of_likes, l.is_verified, l.created_at, l.modified_at, l.is_approved, l.user_id, auth.id as author_id, auth.full_name as author_full_name from artists as a inner join lyrics as l on l.artist_id = a.id inner join artist_slugs as ars on ars.artist_id = a.id inner join lyric_slugs as ls on ls.lyric_id = l.id left join likes on l.id = likes.lyric_id left join authors as auth on l.author_id = auth.id where case when @user_id <> '' then l.user_id = @user_id or a.is_approved = true else l.is_approved = true end and ls.name = @lyric_slug and ars.name = @artist_slug and ars.is_deleted = false and ls.is_deleted = false group by l.id, auth.id, auth.full_name order by number_of_likes;",
      connection);
    ...
    while (await reader.ReadAsync())
    {
      ...
      int? authorId = reader[9] == DBNull.Value ? (int?)null : Convert.ToInt32(reader[9]);
  ...
  public async Task<bool> LyricExistsAsync(int lyricId, string userId)
  {
    bool lyricExists = false;

    string sqlCommand =
      "select exists(select 1 from lyrics where case when @user_id <> '' then user_id = @user_id or is_approved = true else is_approved = true end and is_deleted = false and id = @lyric_id);";
  ...
  public async Task LikeLyricAsync(string userId, int lyricId)
  {
  ...
  public async Task<bool> LyricAlreadyLikedAsync(string userId, int lyricId)
  {
    ...
    string sqlCommand =
      "select exists(select 1 from likes where lyric_id = @lyric_id and user_id = @user_id) as already_liked;";
  ...
  public async Task<LyricSlugCreateResultViewModel> AddLyricSlugAsync(CreateLyricSlugViewModel viewModel)
  {
    ...
    string sqlStatement =
      "insert into lyric_slugs (name, is_primary, created_at, is_deleted, lyric_id) values (@name, @is_primary, @created_at, @is_deleted, @lyric_id) returning name";
  ...
  public async Task<LyricCreateResultViewModel> AddLyricAsync(CreateLyricViewModel viewModel)
  {
    LyricCreateResultViewModel result = new LyricCreateResultViewModel();
    string connectionString = _databaseOptions.ConnectionString;
    string sqlStatement =
      "insert into lyrics (title, body, user_id, created_at, is_deleted, is_approved, artist_id, is_verified) values (@title, @body, @user_id, @created_at, @is_deleted, @is_approved, @artist_id, @is_verified) returning id;";
    ...
    string submitterUsername = await _cognitoService.GetPreferredUsernameAsync(viewModel.UserId);

    ArtistViewModel artist = await _artistsService.GetArtistDetailsByIdAsync(artistId, userId);

    await _emailService.SendLyricSubmissionEmailAsync(submitterUsername, title, result.LyricId,
      artist.FullName, artistId);
